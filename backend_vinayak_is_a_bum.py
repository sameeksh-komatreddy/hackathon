import os, cv2, mediapipe as mp, numpy as np, math, time, json, threading, subprocess, urllib.request, uuid, datetime, io, hashlib, secrets, platform
...
# These need to show up even when BackTrack's browser tab isn't focused —
# i.e. real OS-level notifications, not anything inside the page. We try
# each platform's native mechanism; plyer is used as a portable fallback
...
def send_desktop_notification(title, msg):
   try:
       if _SYSTEM == "Darwin":
           ...
           return
       if _SYSTEM == "Linux":
           subprocess.run(["notify-send", title, msg], check=False, timeout=5)
           return
       if _SYSTEM == "Windows":
           try:
       ...
       try:
           ...
           print(f"no native notifier available ({_SYSTEM}): {e}")
   except Exception as e:
